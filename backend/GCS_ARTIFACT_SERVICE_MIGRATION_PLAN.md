...
**Changes**:
- Add configuration fields for all three services
- Support both full Vertex AI and Express Mode
- Feature flags for gradual rollout

```python
# Artifact Service Configuration
GCS_ARTIFACT_BUCKET: Optional[str] = Field(
    None, 
    description="GCS bucket name for artifact storage (required if USE_GCS_ARTIFACTS=True)"
)
USE_GCS_ARTIFACTS: bool = Field(
    False, 
    description="Use GCS for artifact storage instead of in-memory"
)

# Vertex AI Session Service Configuration
USE_VERTEXAI_SESSIONS: bool = Field(
    False,
    description="Use VertexAiSessionService instead of InMemorySessionService"
)
VERTEXAI_AGENT_ENGINE_ID: Optional[str] = Field(
    None,
    description="Vertex AI Agent Engine ID (required if USE_VERTEXAI_SESSIONS=True). Format: projects/PROJECT_ID/locations/LOCATION/reasoningEngines/ENGINE_ID"
)
# Note: PROJECT_ID and REGION already exist in Settings

# Vertex AI Memory Service Configuration
USE_VERTEXAI_MEMORY: bool = Field(
    False,
    description="Use VertexAiMemoryBankService instead of InMemoryMemoryService"
)
# Uses same VERTEXAI_AGENT_ENGINE_ID as sessions

# Vertex AI Express Mode (Free Tier)
USE_VERTEXAI_EXPRESS_MODE: bool = Field(
    False,
    description="Use Vertex AI Express Mode (free tier, no GCP project required)"
)
# When Express Mode is enabled, PROJECT_ID and REGION are not required
# GOOGLE_API_KEY is used for authentication
```
...
# Vertex AI Sessions
USE_VERTEXAI_SESSIONS=false
VERTEXAI_AGENT_ENGINE_ID=projects/YOUR_PROJECT_ID/locations/YOUR_REGION/reasoningEngines/YOUR_ENGINE_ID

# Vertex AI Memory
USE_VERTEXAI_MEMORY=false

# Express Mode (Alternative - Free Tier)
USE_VERTEXAI_EXPRESS_MODE=false
# If Express Mode is true, set GOOGLE_GENAI_USE_VERTEXAI=TRUE
```

### Phase 2: Infrastructure Setup

#### 2.1 Prerequisites
**Option A: Full Vertex AI (Production)**
- GCP project with billing enabled
- Vertex AI API enabled
- Vertex AI Agent Engine created (Reasoning Engine)
- GCS bucket for artifacts

**Option B: Vertex AI Express Mode (Free Tier)**
- Eligible Gmail account
- Google API key
- No GCP project required

#### 2.2 Create Vertex AI Agent Engine (Full Vertex AI Only)
...
#### 2.4 IAM Permissions (Full Vertex AI Only)
...
try:
    ...
    VertexAiSessionService = None  # Graceful fallback if not installed
...
try:
    ...
    VertexAiMemoryBankService = None  # Graceful fallback if not installed
...
def _create_artifact_service(self) -> BaseArtifactService:
    """Create appropriate artifact service based on configuration."""
    settings = get_settings()
    
    if settings.USE_GCS_ARTIFACTS:
        if not settings.GCS_ARTIFACT_BUCKET:
            raise ValueError(
                "GCS_ARTIFACT_BUCKET must be set when USE_GCS_ARTIFACTS=True"
            )
        try:
...
def _create_session_service(self):
    ...
    if settings.USE_VERTEXAI_SESSIONS:
        if VertexAiSessionService is None:
            logger.error("VertexAiSessionService not available. Install google-adk[vertexai]")
            logger.warning("Falling back to InMemorySessionService")
            return InMemorySessionService()
        
        if settings.USE_VERTEXAI_EXPRESS_MODE:
            # Express Mode: Only requires agent_engine_id
            if not settings.VERTEXAI_AGENT_ENGINE_ID:
                raise ValueError(
                    "VERTEXAI_AGENT_ENGINE_ID must be set when USE_VERTEXAI_EXPRESS_MODE=True"
                )
            try:
                return VertexAiSessionService(agent_engine_id=settings.VERTEXAI_AGENT_ENGINE_ID)
            except Exception as e:
                logger.error(f"Failed to initialize VertexAiSessionService (Express Mode): {e}")
                logger.warning("Falling back to InMemorySessionService")
                return InMemorySessionService()
        else:
            # Full Vertex AI: Requires project, location, and agent_engine_id
            if not settings.VERTEXAI_AGENT_ENGINE_ID:
                raise ValueError(
                    "VERTEXAI_AGENT_ENGINE_ID must be set when USE_VERTEXAI_SESSIONS=True"
                )
            try:
...
def _create_memory_service(self):
    """Create appropriate memory service based on configuration."""
    settings = get_settings()
    
    if settings.USE_VERTEXAI_MEMORY:
        if VertexAiMemoryBankService is None:
            logger.error("VertexAiMemoryBankService not available. Install google-adk[vertexai]")
        ...
        if settings.USE_VERTEXAI_EXPRESS_MODE:
            ...
            try:
                return VertexAiMemoryBankService(agent_engine_id=settings.VERTEXAI_AGENT_ENGINE_ID)
            except Exception as e:
                logger.error(f"Failed to initialize VertexAiMemoryBankService (Express Mode): {e}")
                logger.warning("Falling back to InMemoryMemoryService")
                return InMemoryMemoryService()
        else:
            # Full Vertex AI: Requires project, location, and agent_engine_id
            if not settings.VERTEXAI_AGENT_ENGINE_ID:
                raise ValueError(
                    "VERTEXAI_AGENT_ENGINE_ID must be set when USE_VERTEXAI_MEMORY=True"
                )
            try:
                ...
                logger.error(f"Failed to initialize VertexAiMemoryBankService: {e}")
                logger.warning("Falling back to InMemoryMemoryService")
                return InMemoryMemoryService()
    else:
        return InMemoryMemoryService()
...
def _get_app_name(self) -> str:
    """Get app_name for Runner based on session service type.
    
    For VertexAiSessionService, app_name should be the Reasoning Engine resource name.
    For InMemorySessionService, app_name is the agent name.
    """
    settings = get_settings()
    
    if settings.USE_VERTEXAI_SESSIONS and settings.VERTEXAI_AGENT_ENGINE_ID:
        # Use Reasoning Engine resource name as app_name
        return settings.VERTEXAI_AGENT_ENGINE_ID
    else:
        # Use agent name for in-memory services
...
def __init__(
    ...
    # Initialize ADK Runner with configured services
    self.runner = Runner(
        app_name=app_name,
        agent=self.agent,
...
- If GCS initialization fails, fall back to `InMemoryArtifactService`
- Log warnings but don't fail application startup
- Allow runtime switching via configuration

#### 4.2 Error Handling
**File**: `backend/app/agent_executor.py`

**Add**:
- Try-catch around `GcsArtifactService` initialization
- Log errors with context (bucket name, error message)
- Provide clear error messages if bucket doesn't exist or permissions are missing
...
1. Test `_create_artifact_service()` with `USE_GCS_ARTIFACTS=False` → returns `InMemoryArtifactService`
2. Test `_create_artifact_service()` with `USE_GCS_ARTIFACTS=True` and valid bucket → returns `GcsArtifactService`
3. Test `_create_artifact_service()` with missing bucket → raises `ValueError`
4. Test `_create_artifact_service()` with GCS initialization failure → falls back to `InMemoryArtifactService`

**Test Cases for Session Service**:
5. Test `_create_session_service()` with `USE_VERTEXAI_SESSIONS=False` → returns `InMemorySessionService`
6. Test `_create_session_service()` with Express Mode → returns `VertexAiSessionService` with Express Mode config
7. Test `_create_session_service()` with Full Vertex AI → returns `VertexAiSessionService` with project/location
8. Test `_create_session_service()` with missing agent_engine_id → raises `ValueError`
9. Test `_create_session_service()` with Vertex AI initialization failure → falls back to `InMemorySessionService`
10. Test `_create_session_service()` when VertexAiSessionService not installed → falls back gracefully

**Test Cases for Memory Service**:
11. Test `_create_memory_service()` with `USE_VERTEXAI_MEMORY=False` → returns `InMemoryMemoryService`
12. Test `_create_memory_service()` with Express Mode → returns `VertexAiMemoryBankService` with Express Mode config
13. Test `_create_memory_service()` with Full Vertex AI → returns `VertexAiMemoryBankService` with project/location
14. Test `_create_memory_service()` with missing agent_engine_id → raises `ValueError`
15. Test `_create_memory_service()` with Vertex AI initialization failure → falls back to `InMemoryMemoryService`

**Test Cases for App Name**:
16. Test `_get_app_name()` with Vertex AI sessions → returns `VERTEXAI_AGENT_ENGINE_ID`
17. Test `_get_app_name()` with in-memory sessions → returns agent name

**Test Cases for Runner Initialization**:
18. Test that Runner is initialized with correct services based on configuration
19. Test that Runner uses correct app_name based on session service type
...
4. Set environment variables:
   ...
   export USE_GCS_ARTIFACTS=true
   export GCS_ARTIFACT_BUCKET=test-artifacts-bucket
   export USE_VERTEXAI_SESSIONS=true
   export USE_VERTEXAI_MEMORY=true
...
**Option B: Express Mode Setup (Free Tier)**
1. Sign up for Vertex AI Express Mode with eligible Gmail account
2. Get Google API key from Express Mode
3. Set environment variables:
   ```bash
   export GOOGLE_API_KEY=your-express-mode-api-key
   export GOOGLE_GENAI_USE_VERTEXAI=TRUE
   export USE_VERTEXAI_EXPRESS_MODE=true
   export USE_VERTEXAI_SESSIONS=true
   export USE_VERTEXAI_MEMORY=true
   export VERTEXAI_AGENT_ENGINE_ID=your-express-mode-engine-id
   export USE_GCS_ARTIFACTS=false  # Express Mode doesn't support GCS artifacts
   ```
4. Run application and verify:
   - Sessions persist (via Express Mode)
   - Memory is searchable (via Express Mode)

### Phase 6: Deployment Strategy

#### 6.1 Gradual Rollout Plan

**Phase 1: Artifacts Only (Lowest Risk)**
1. Deploy with all Vertex AI flags `false` (no changes)
2. Create GCS bucket and set permissions
3. Enable `USE_GCS_ARTIFACTS=true` in staging
4. Monitor for 1-2 weeks
5. Enable in production

**Phase 2: Sessions (Medium Risk)**
1. Create Vertex AI Agent Engine
2. Set up IAM permissions
3. Enable `USE_VERTEXAI_SESSIONS=true` in staging
4. Verify session persistence across restarts
5. Monitor for 1-2 weeks
6. Enable in production

**Phase 3: Memory (Higher Risk - New Feature)**
1. Enable `USE_VERTEXAI_MEMORY=true` in staging
2. Test memory search functionality
3. Verify cross-session memory retrieval
4. Monitor memory usage and costs
5. Enable in production after thorough testing

**Phase 4: Cleanup**
1. Remove fallback code once all services are stable
2. Update documentation
3. Remove feature flags (optional)
...
### Configuration
- [ ] Add `GCS_ARTIFACT_BUCKET` to `Settings` class
- [ ] Add `USE_GCS_ARTIFACTS` flag to `Settings` class
- [ ] Add `USE_VERTEXAI_SESSIONS` flag to `Settings` class
- [ ] Add `USE_VERTEXAI_MEMORY` flag to `Settings` class
- [ ] Add `USE_VERTEXAI_EXPRESS_MODE` flag to `Settings` class
- [ ] Add `VERTEXAI_AGENT_ENGINE_ID` to `Settings` class
- [ ] Update `.env.example` with new variables
- [ ] Install `google-adk[vertexai]` package

### Infrastructure
- [ ] Enable Vertex AI API (if using full Vertex AI)
...
### Code Changes
- [ ] Import all required services (`GcsArtifactService`, `VertexAiSessionService`, `VertexAiMemoryBankService`)
- [ ] Create `_create_artifact_service()` factory method
- [ ] Create `_create_session_service()` factory method
- [ ] Create `_create_memory_service()` factory method
- [ ] Create `_get_app_name()` method (for Reasoning Engine resource name)
- [ ] Update `__init__` to use all factory methods
- [ ] Add error handling and fallback logic for all services
- [ ] Add logging for service initialization
- [ ] Update `SessionManager` if needed (app_name handling)
...
- [ ] Test locally with ADC (full Vertex AI)
- [ ] Test locally with Express Mode (free tier)
- [ ] Test in staging environment
- [ ] Verify fallback to in-memory services works for all services
- [ ] Test error scenarios (missing config, API failures, etc.)

### Deployment
- [ ] Deploy to staging with all flags `false` (verify no changes)
- [ ] Phase 1: Enable GCS artifacts in staging, monitor, then production
- [ ] Phase 2: Enable Vertex AI sessions in staging, monitor, then production
- [ ] Phase 3: Enable Vertex AI memory in staging, monitor, then production
...
- [ ] Add Vertex AI Express Mode setup guide
- [ ] Document Reasoning Engine creation process
- [ ] Add troubleshooting guide for all services
- [ ] Update deployment guide with gradual rollout steps
...
**Artifacts (GCS)**:
1. **GCS Bucket Not Found**: Fallback to InMemoryArtifactService
2. **Permission Errors**: Clear error messages, fallback mechanism
3. **Cost Overruns**: Monitor storage usage, set lifecycle policies
4. **Performance Issues**: Monitor latency, consider caching

**Sessions (Vertex AI)**:
5. **Agent Engine Not Found**: Clear error, fallback to InMemorySessionService
6. **API Quota Exceeded**: Monitor usage, fallback mechanism
...
**Memory (Vertex AI)**:
9. **Memory Search Failures**: Fallback to InMemoryMemoryService
...
**Express Mode**: Free tier available for eligible accounts

### Migration Timeline Estimate

- **Phase 1 (Artifacts)**: 1-2 weeks (setup + testing + rollout)
- **Phase 2 (Sessions)**: 2-3 weeks (setup + testing + rollout)
- **Phase 3 (Memory)**: 2-3 weeks (setup + testing + rollout)
...
- Free tier eligible
- Don't have GCP project setup
- Want to test Vertex AI features quickly

**Use Full Vertex AI If**:
- Production environment
- Need full control and monitoring
- Have GCP project with billing
- Need integration with other GCP services
...
- [Vertex AI Express Mode](https://google.github.io/adk-docs/sessions/memory/#vertex-ai-express-mode)
